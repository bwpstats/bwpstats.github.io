...
import { Button, ExternalLink, LoadingSpinner, PlayerHead, PlayerName, 
	ReactIcon, SortableList } from 'src/components';
import { APP } from 'src/constants/app';
import { useAPIContext } from 'src/hooks';
import * as Utils from 'src/utils';
import { getClientHeaders, httpGet } from 'src/utils';
import { getPlayerRankPriority, getGuildMemberRank } from 'src/utils/hypixel';
...
export function GuildMemberList() {
	...
	function sortByGuildRank(memberList, polarity) {
		return memberList.sort((a,b) => {
			const aRank = getGuildMemberRank(a);
			const bRank = getGuildMemberRank(b);
	...
	function sortByJoinDate(memberList, polarity) {
		return memberList.sort((a,b) => a.time > b.time ? 
			polarity : -polarity);
	}
	...
	function exportGuildXLSX() {
		...
		const columns = [
			...
			{title: "Joined Since", from: (m) => Utils.dateFormat(m.time * 1000)},
		]
						...
						<tr key={member.time}>
							...
							<td className="text-shadow">
								<Link to={`/player/${data.username}`}>
									<PlayerName username={data.username} voxyl={data} size="lg"></PlayerName>
								</Link>
							</td>
							<td>{getGuildMemberRank(member, guild.ranks).name}</td>
							<td>{Utils.dateFormat(member.time * 1000)}</td>
						</tr>
						);
				}}
			</SortableList>
