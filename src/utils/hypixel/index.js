 ...
 */
export function getGuildMemberRank(member) {
	// hardcoded because api doesnt provide anything + users can only be these roles in a guild
	const rankJson = {
		MEMBER: {name: 'Member', priority: 0},
		MODERATOR: {name: 'Moderator', priority: 1},
		ADMIN: {name: 'Admin', priority: 2},
		OWNER: {name: 'Owner', priority: 3},
	}
	return rankJson[member.role] || {name: member.role, priority: 0};
}
